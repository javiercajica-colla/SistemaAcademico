...
class _CoursesScreenState extends State<CoursesScreen> with SingleTickerProviderStateMixin {
                              ...
                              children: [
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: AppColors.coordinator.withValues(alpha: 0.1),
                                    borderRadius: BorderRadius.circular(8),
                                  ),
                                  child: Center(
                                    child: Text(c.grade, style: const TextStyle(color: AppColors.coordinator, fontWeight: FontWeight.bold, fontSize: 16)),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('$count estudiantes', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                                    ],
                                  ),
                                ),
                              ],
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text('Selecciona un curso', style: TextStyle(color: AppColors.textSecondary, fontSize: 16)),
        ],
      ),
    );
  }
  ...
  Widget _buildCourseDetail(BuildContext context, AcademicProvider academic) {
    ...
    final teacher = academic.teacherById(course.directorTeacherId ?? '');
    final subjectAssignments = academic.assignments.where((a) => a.courseId == course.id).toList();
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Grado ${course.grade}° • Sección ${course.section}', style: const TextStyle(color: AppColors.textSecondary)),
                          ],
                        ),
                      ),
                      ElevatedButton.icon(
                ...
                const TabBar(tabs: [Tab(text: 'Estudiantes'), Tab(text: 'Asignaturas')]),
              ],
            ),
          ),
          Expanded(
  ...
  Widget _buildStudentsList(students, AcademicProvider academic) {
    if (students.isEmpty) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) {
        ...
        return ListTile(
          ...
          trailing: avg > 0 ? GradeChip(grade: avg) : const Text('Sin calificaciones', style: TextStyle(color: AppColors.textTertiary, fontSize: 11)),
        );
      },
    );
  }

  Widget _buildSubjectsList(List subjectAssignments, AcademicProvider academic) {
    return ListView.separated(
      padding: const EdgeInsets.all(24),
      ...
      itemBuilder: (_, i) {
        ...
        return ListTile(
          ...
          title: Text(subject?.name ?? 'Asignatura', style: const TextStyle(fontWeight: FontWeight.w500)),
          subtitle: Text(subject?.area ?? ''),
          trailing: teacher != null
              ? Chip(label: Text(teacher.fullName, style: const TextStyle(fontSize: 11)), backgroundColor: AppColors.teacher.withValues(alpha: 0.08))
              : const Text('Sin docente', style: TextStyle(color: AppColors.textTertiary, fontSize: 11)),
        );
      },
    );
  }
          ...
          child: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                items: ['6', '7', '8', '9', '10', '11'].map((g) => DropdownMenuItem(value: g, child: Text('Grado $g°'))).toList(),
                onChanged: (_) {},
              ),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                decoration: const InputDecoration(labelText: 'Sección'),
                items: ['A', 'B', 'C'].map((s) => DropdownMenuItem(value: s, child: Text('Sección $s'))).toList(),
                onChanged: (_) {},
              ),
            ],
          ),
  ...
  void _showEnrollDialog(BuildContext context, AcademicProvider academic) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: DropdownButtonFormField<String>(
            ...
            items: academic.students
                .where((s) => s.courseId == null || s.courseId != _selectedCourse)
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancelar')),
          ElevatedButton(onPressed: () => Navigator.pop(ctx), child: const Text('Matricular')),
        ],
      ),
    );
  }
