...
class _ObservationsScreenState extends State<ObservationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final allObs = academic.observations
        .where((o) => o.teacherId == teacher.id)
        .toList();
    final filtered = allObs.where((o) {
      ...
      if (_search.isNotEmpty) {
        ...
        if (!student.fullName.toLowerCase().contains(_search) &&
      ...
      children: [
        Expanded(
          child: TextField(
            ...
            decoration: const InputDecoration(
              hintText: 'Buscar por estudiante o título...',
        ...
        SegmentedButton<String>(
          segments: const [
            ...
            ButtonSegment(
              value: 'positive',
              icon: Icon(Icons.thumb_up_rounded, size: 14),
              label: Text('Positiva'),
            ),
            ButtonSegment(
              value: 'academic',
            ...
            ButtonSegment(
              value: 'disciplinary',
              icon: Icon(Icons.warning_rounded, size: 14),
              label: Text('Disciplinaria'),
            ),
          ],
          selected: {_filter},
  ...
  Widget _buildObsCard(Observation obs, AcademicProvider academic) {
    final student = academic.students.firstWhere(
      (s) => s.id == obs.studentId,
      orElse: () => academic.students.first,
    );
    final subject = obs.subjectId != null
        ? academic.subjectById(obs.subjectId!)
        : null;
              ...
              child: Column(
                ...
                children: [
                  ...
                  SegmentedButton<ObservationType>(
                    segments: const [
                      ButtonSegment(
                        value: ObservationType.positive,
                        label: Text('Positiva'),
                      ),
                      ButtonSegment(
                      ...
                      ButtonSegment(
                        value: ObservationType.disciplinary,
                        label: Text('Disciplinaria'),
                      ),
                    ],
                    selected: {selectedType},
                    onSelectionChanged: (s) =>
                        setS(() => selectedType = s.first),
                  ),
                  const SizedBox(height: 12),
                  TextField(
              ...
              onPressed: () {
                if (selectedStudent != null &&
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Observación registrada'),
